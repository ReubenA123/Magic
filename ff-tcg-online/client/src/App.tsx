...
export default function App() {
  ...
  function handleLogin(name: string, password: string) {
    ...
    socket.on('action-error', (message: string) => {
      ...
      // Errors are transient UI feedback, not something that should stick
      // around forever - clear it after a few seconds.
      setTimeout(() => setActionError(null), 4000);
    });
  }

  // Clean up listeners if the component ever unmounts (e.g. hot reload in dev).
  useEffect(() => {
    return () => {
      try {
